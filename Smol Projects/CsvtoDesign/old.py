...
def create_compact_pdf_from_csv(csv_file, pdf_file):
    ...
    # Create paragraph styles for text wrapping with center alignment
    ...
    for page in range(total_pages):
        ...
        for i in range(start_index, end_index):
            ...
            avail_width = box_width - 10  # 5px padding on each side
            ...
            name_para.drawOn(c, x + 5, y + box_height - 15 - name_height)

            # No line separator between name and university as requested

            # Draw university with text wrapping
            univ_width, univ_height = univ_para.wrap(avail_width, box_height +30 / 2)
            univ_para.drawOn(c, x + 5, y + box_height / 2 - univ_height)
    ...
    c.save()
    print(f"PDF successfully created: {pdf_file} with {total_pages} pages")
